...
public class OpenInv extends PluginBase {
    ...
    public void sendEnderChestInventory(Player player){
	    ...
	    nbt = new CompoundTag("", [ new ListTag("Items", []), new StringTag("id", BlockEntity.ENDER_CHEST), new IntTag("x", floor(player.x)), new IntTag("y", floor(player.y) - 2), new IntTag("z", floor(player.z)) ]);
